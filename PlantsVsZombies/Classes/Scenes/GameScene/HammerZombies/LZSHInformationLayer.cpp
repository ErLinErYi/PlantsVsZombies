...
void HInformationLayer::showUserText()
{
	...
	username->setPosition(Vec2(Director::getInstance()->getWinSize().width / 2.0f, 900));
...
void HInformationLayer::showTipsText()
{
	...
	tips->setString(_global->userInformation->getGameText().find("锤僵尸小提示" + to_string(rand() % 2 + 1))->second->text);
	tips->setFontSize(30);
	tips->setColor(Color3B::MAGENTA);
	tips->enableGlow(Color4B::GREEN);
	tips->setPosition(Vec2(Director::getInstance()->getWinSize().width / 2.0f, 35));
